...
[360, 480, 640, 800, 1024].forEach(function(size) {
  var resizeImageTask = 'resize_' + size;
  gulp.task(resizeImageTask, function(done) {
    gulp
      .src(project.buildSrc + '/images/*')
      .pipe(parallel(imageResize({ width: size }), os.cpus().length))
      .pipe(
...
gulp.task(
  'images',
  gulp.parallel(resizeImageTasks, function copyOriginalImages(done) {
    gulp
      .src(project.buildSrc + '/images/*')
      .pipe(gulp.dest(project.buildDest + '/images'));
    done();
